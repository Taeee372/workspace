    ...
    <div className={styles.container}>
      ...
      <div className={styles.reg_form_div}>
        <div>
          <div>
            <span>제조사</span>
            <Select size='130px' name='brand' value={car.brand} 
                    onChange={e => {
                      handleCar(e);
                      setErrorMsg({
                        ...errorMsg,
          ...
          <div>
            <span>모델명</span>
            <Input size='130px' name='modelName' value={car.modelName} 
                  onChange={e => {
                    handleCar(e);
                    setErrorMsg({
                    ...errorMsg,
                    'modelName' : e.target.value === '' ? '모델명 입력은 필수입니다.' : ''
                    });
                  }} 
                  onKeyDown={e => {
                    if(e.key === 'Enter') regCar()
                  }}
            />
          ...
          <div>
            <span>차량가격</span>
            <Input size='130px' name='price' value={car.price} 
                  onChange={e => {
                    handleCar(e);
                    if(e.target.value === ''){
                      setErrorMsg({
