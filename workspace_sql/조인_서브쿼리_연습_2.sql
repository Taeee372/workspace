...
# -서브쿼리
SELECT (SELECT TITLE FROM TEST_BOOK WHERE BOOK_ID = RENTAL.BOOK_ID) AS TITLE
	, (SELECT AUTHOR FROM TEST_BOOK WHERE BOOK_ID = RENTAL.BOOK_ID) AS AUTHOR
	, BORROWER_NAME
	, RENTAL_DATE
FROM RENTAL;
...
FROM RENTAL R INNER JOIN TEST_BOOK T
...
# -TEST_BOOK 테이블 기준
SELECT TITLE
	, (SELECT COUNT(BOOK_ID) FROM RENTAL WHERE BOOK_ID = TEST_BOOK.BOOK_ID) AS '대출 횟수'
	, CASE (SELECT COUNT(RENTAL_ID) FROM RENTAL WHERE BOOK_ID = TEST_BOOK.BOOK_ID)
		WHEN 0 THEN '대출이력없음'
		ELSE '대출이력있음' END AS '대출 여부'
FROM TEST_BOOK; 
