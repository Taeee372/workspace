...
# Foreign Key 사용법 : REFERENCES 참조할 테이블명(참조할 컬럼)
# 두 테이블간의 잘못된 데이터 변경을 막음

DELETE FROM BOARD_MEMBER
WHERE MEM_ID = 'aaa'; # 안 지워짐
...
FROM BOARD_MEMBER M INNER JOIN FREE_BOARD B
...
SELECT BOARD_NUM, TITLE, CREATE_DATE, M.MEM_ID, MEM_AGE
FROM BOARD_MEMBER M INNER JOIN FREE_BOARD B
ON M.MEM_ID = B.MEM_ID
